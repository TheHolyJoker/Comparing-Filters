...
// static constexpr uint64_t mask_Table[50] = {
//         07,
//         070,
//         0700,
//         07000,
...
static constexpr uint64_t simd_table[50][4] = {
        {07ULL, 0, 0, 0},
        {070ULL, 0, 0, 0},
        {0700ULL, 0, 0, 0},
        {07000ULL, 0, 0, 0},
        ...
        {070000000ULL, 0, 0, 0},
        {0700000000ULL, 0, 0, 0},
        {07000000000ULL, 0, 0, 0},
        {070000000000ULL, 0, 0, 0},
        {0700000000000ULL, 0, 0, 0},
        {07000000000000ULL, 0, 0, 0},
        {070000000000000ULL, 0, 0, 0},
        {0700000000000000ULL, 0, 0, 0},
        {07000000000000000ULL, 0, 0, 0},
        {070000000000000000ULL, 0, 0, 0},
        {0700000000000000000ULL, 0, 0, 0},
        {07000000000000000000ULL, 0, 0, 0},
        {070000000000000000000ULL, 0, 0, 0},
        {0700000000000000000000ULL, 0, 0, 0},
        {0, 07ULL, 0, 0},
        {0, 070ULL, 0, 0},
        {0, 0700ULL, 0, 0},
        {0, 07000ULL, 0, 0},
        ...
        {0, 070000000ULL, 0, 0},
        {0, 0700000000ULL, 0, 0},
        {0, 07000000000ULL, 0, 0},
        {0, 070000000000ULL, 0, 0},
        {0, 0700000000000ULL, 0, 0},
        {0, 07000000000000ULL, 0, 0},
        {0, 070000000000000ULL, 0, 0},
        {0, 0700000000000000ULL, 0, 0},
        {0, 07000000000000000ULL, 0, 0},
        {0, 070000000000000000ULL, 0, 0},
        {0, 0700000000000000000ULL, 0, 0},
        {0, 07000000000000000000ULL, 0, 0},
        {0, 070000000000000000000ULL, 0, 0},
        {0, 0700000000000000000000ULL, 0, 0},
        {0, 0, 07ULL, 0},
        {0, 0, 070ULL, 0},
        {0, 0, 0700ULL, 0},
        {0, 0, 07000ULL, 0},
...
template<typename spareItemType, typename itemType>
...
public:
    ...
    inline auto lookup(const itemType s) const -> bool {
        // return minimal_lookup(s);
        // return lookup_ver1(s);
        // return lookup_ver2(s);
        // return lookup_ver3(s);
        // return lookup_ver4(s);
        // return lookup_ver5(s);
        // return lookup_ver8(s);
        return lookup_ver9(s);
    }
...
*/
    ...
    inline auto lookup_ver3(const itemType s) const -> bool {
        ...
        //        uint64_t *header = (ind) ? packed_fpd_array[pd_index].header2 : packed_fpd_array[pd_index].header1;
        // if (counter)
        // const uint64_t v = Fixed_pd45::Body::get_v(rem, &pd_array[pd_index]) >> 19ul;
        // if (!v) {
        //     return (counter == Fixed_pd45::counter_overflowed_val) ? spare->find(((uint64_t) pd_index << (quotient_length + bits_per_item)) | (quot << bits_per_item) | rem) : false;
        // } else {
        //     if (counter == Fixed_pd45::counter_overflowed_val)
        //         return spare->find(((uint64_t) pd_index << (quotient_length + bits_per_item)) | (quot << bits_per_item) | rem);
        //     return (v >> Fixed_pd45::Header::get_start(quot, &pd_array[pd_index])) & MASK(counter);
        // }
        // if (!v)
        // return
        // const uint8_t counter = Fixed_pd45::Header::read_counter(quot, &pd_array[pd_index]);
        //
        // return Fixed_pd45::Body::get_v(rem, &pd_array[pd_index]) && Fixed_pd45::Header::get_start(quot, &pd_array[pd_index]);
        // const uint64_t v = Fixed_pd45::Body::get_v(rem, &pd_array[pd_index])
        // if (!v){
        // return
        // }
        // const uint64_t start = Fixed_pd45::Header::get_start(quot, &pd_array[pd_index]);
        // return (counter < 7) ? counter && v && ((v >> start) & (1ULL << counter) - 1ULL) : spare->find(((uint64_t) pd_index << (quotient_length + bits_per_item)) | (quot << bits_per_item) | rem);
        //
        // size_t counter = Fixed_pd45::Header::read_counter(quot, &pd_array[pd_index]);
        // if (!counter) {
        //     return false;
    ...
    inline auto lookup_ver4(const itemType s) const -> bool {
        ...
        if (!v) {
            const uint64_t quot = reduce32((uint32_t) out2, (uint32_t) quot_range);
            size_t counter = Fixed_pd45::Header::read_counter(quot, &pd_array[pd_index]);
            return (counter == Fixed_pd45::counter_overflowed_val) &&
    ...
    // inline auto lookup_ver5(const itemType s) const -> bool {
    //     uint64_t hash_res = Hasher(s);
    //     uint32_t out1 = hash_res >> 32u, out2 = hash_res & MASK32;
    //     const uint32_t pd_index = reduce32(out1, (uint32_t) number_of_pd);
    //     const uint64_t quot = reduce32((uint32_t) out2, (uint32_t) quot_range);
    //     const uint64_t mask = mask_Table[quot];


    //     const uint64_t word = ((uint64_t *) (&pd_array[pd_index]))[Fixed_pd45::word_index_Table[quot]];
    //     // size_t counter = Fixed_pd45::Header::read_counter(quot, &pd_array[pd_index]);
    //     // if (counter == 0) {
    //     //     assert(!(mask & word));
    //     // }
    ...
    //         return false;
    //     } else if ((mask & word) != mask) {
    //         // std::cout << "h2" << std::endl;
    //         // if (counter >= 4)
    //         //     std::cout << "counter: " << counter << std::endl;
    //         // assert(counter);
    //         // assert(counter != 7);
    //         const uint8_t rem = out2 & 255ul;
    //         uint64_t v = Fixed_pd45::Body::get_v(rem, &pd_array[pd_index]);
    //         if (!v)
    //             return false;
    //         uint64_t start = Fixed_pd45::Header::get_start(quot, &pd_array[pd_index]);
    //         //error here
    //         return (v >> start) & MASK(Fixed_pd45::Header::read_counter(quot, &pd_array[pd_index]));
    //     } else {
    ...
    inline auto lookup_ver6(const itemType s) const -> bool {
        uint64_t hash_res = Hasher(s);
        uint32_t out1 = hash_res >> 32u, out2 = hash_res & MASK32;
        const uint32_t pd_index = reduce32(out1, (uint32_t) number_of_pd);
        const uint64_t quot = reduce32((uint32_t) out2, (uint32_t) quot_range);
    ...
    inline auto lookup_helper(size_t counter, uint64_t quot, uint8_t rem, const __m512i *pd) const -> bool {
        const uint64_t v = Fixed_pd45::Body::get_v(rem, pd) >> 19ul;
        if (!v)
            return false;
        return (v >> Fixed_pd45::Header::get_start(quot, pd)) & MASK(counter);
    }

    inline auto lookup_ver7(const itemType s) const -> bool {
        uint64_t hash_res = Hasher(s);
        uint32_t out1 = hash_res >> 32u, out2 = hash_res & MASK32;
        const uint32_t pd_index = reduce32(out1, (uint32_t) number_of_pd);
        const uint64_t quot = reduce32((uint32_t) out2, (uint32_t) quot_range);
        ...
        switch (counter) {
            case 0:
                return false;
            case 1:
                return lookup_helper(counter, quot, rem, &pd_array[pd_index]);
            case 2:
                return lookup_helper(counter, quot, rem, &pd_array[pd_index]);
            case 3:
                return lookup_helper(counter, quot, rem, &pd_array[pd_index]);
    ...
    inline auto lookup_ver8(const itemType s) const -> bool {
        uint64_t hash_res = Hasher(s);
        uint32_t out1 = hash_res >> 32u, out2 = hash_res & MASK32;
        const uint32_t pd_index = reduce32(out1, (uint32_t) number_of_pd);
        const uint64_t quot = reduce32((uint32_t) out2, (uint32_t) quot_range);
        ...
        if (CMP(counter)) {
            ...
            const uint8_t rem = out2 & MASK(bits_per_item);
            return counter && spare->find(
                                      ((uint64_t) pd_index << (quotient_length + bits_per_item)) | (quot << bits_per_item) | rem);
        }
    }

    inline auto lookup_ver9(const itemType s) const -> bool {
        uint64_t hash_res = Hasher(s);
        uint32_t out1 = hash_res >> 32u, out2 = hash_res & MASK32;
        const uint32_t pd_index = reduce32(out1, (uint32_t) number_of_pd);
        const uint64_t quot = reduce32((uint32_t) out2, (uint32_t) quot_range);
        ...
        // return Fixed_pd45::Header::read_counter4(quot, &pd_array[pd_index]);
        __m256i pd = _mm512_castsi512_si256(pd_array[pd_index]);
        __m256i mask = _mm256_loadu_si256((__m256i *) simd_table[quot]);
        if (!_mm256_testc_si256(pd, mask)) {
            uint64_t v = Fixed_pd45::Body::get_v(rem, &pd_array[pd_index]);
            if (!v)
                return false;
            uint64_t start = Fixed_pd45::Header::get_start(quot, &pd_array[pd_index]);
    ...
    inline auto minimal_lookup(const itemType s) const -> bool {
        ...
        // const uint64_t v = Fixed_pd45::Body::get_v(rem, &pd_array[pd_index]);
        // if (!v) {
        //     return _mm256_testc_si256(pd, mask) &&
        //            spare->find(((uint64_t) pd_index << (quotient_length + bits_per_item)) | (quot << bits_per_item) | rem);
        // }
        // uint64_t start = Fixed_pd45::Header::get_start(quot, &pd_array[pd_index]);
        // return (v >> start) & MASK(Fixed_pd45::Header::read_counter4(quot, &pd_array[pd_index]));
        // return 
        // return Fixed_pd45::Body::get_v(rem, &pd_array[pd_index]) || _mm256_testc_si256(pd, mask);
        // return Fixed_pd45::Body::get_v(rem, &pd_array[pd_index]) && _mm256_testc_si256(pd, mask);
        // if (!_mm256_testc_si256(pd, mask)) {
        //     uint64_t v = Fixed_pd45::Body::get_v(rem, &pd_array[pd_index]);
        //     if (!v)
        ...
        //             ((uint64_t) pd_index << (quotient_length + bits_per_item)) | (quot << bits_per_item) | rem);
        // }
        // __m256i res = _mm256_and_si256(pd, mask);
        // return _mm256_testz_si256(res, res);
        // return Fixed_pd45::Header::read_TWO_counter(quot, &pd_array[pd_index]);
        // return Fixed_pd45::Header::read_one_bit(quot, &pd_array[pd_index]);
        // return Fixed_pd45::Body::get_v(rem, &pd_array[pd_index]);
    }
    ...
    auto lookup_count(const itemType s, int caser = 0) const -> bool {
        constexpr bool x = CMP(1) && CMP(2) && CMP(3) && CMP(4) && CMP(5) && CMP(6);
        constexpr bool y = CMP(0) || CMP(7);
        static_assert(x != y);
        static_assert(x);
        // static_assert(y);
        ...
        total++;
        static size_t v_disq = 0;
        static size_t counter_disq = 0;
        static size_t v_minus_c = 0;
        static size_t v_and_c_OF = 0;

        // static size_t shifted_v_disq = 0;

        // static size_t only_v_disq = 0;
        // static size_t only_counter_disq = 0;

        // static size_t counter_and_sv_disq = 0;

        // static size_t v_disqualified_but_should_look_in_l2 = 0;
        // static size_t v_disqualified_but_l2_contained = 0;
        // static size_t v_disqualified_and_l2_disq = 0;


        // static size_t v_arr[7] = {0};
        // static size_t c_arr[7] = {0};
        ...
        if (caser == 2) {
            ...
            std::cout << "v_disq:          \t" << v_disq;
            std::cout << "    \t" << (1.0 * v_disq / total) << std::endl;
            std::cout << "counter_disq:    \t" << counter_disq;
            std::cout << "    \t" << (1.0 * counter_disq / total) << std::endl;
            std::cout << "v_minus_c:       \t" << v_minus_c;
            std::cout << "    \t" << (1.0 * v_minus_c / total) << std::endl;
            size_t v_cup_c = v_disq - v_minus_c;
            size_t c_minus_v = counter_disq - v_cup_c;
            std::cout << "c_minus_v:       \t" << c_minus_v;
            std::cout << "    \t" << (1.0 * c_minus_v / total) << std::endl;
            std::cout << "v_and_c_OF:      \t" << v_and_c_OF;
            std::cout << "    \t" << (1.0 * v_and_c_OF / total) << std::endl;
            std::cout << line << std::endl;
            return true;
        }
        ...
        const __m512i *ppd = &pd_array[pd_index];
        size_t counter = Fixed_pd45::Header::read_counter(quot, ppd);
        uint64_t v = Fixed_pd45::Body::get_v(rem, ppd);

        counter_disq += (counter == 0);
        v_disq += (!v);
        v_minus_c += (!v) && (counter != 0);
        v_and_c_OF += (!v) && (counter == 7);
