...
public:
    Fixed_Dict(size_t max_number_of_elements, double level1_load_factor, double level2_load_factor)
        : filter_max_capacity(max_number_of_elements),
          ...
          pd_index_length(ceil_log2(compute_number_of_PD(max_number_of_elements, max_capacity * 2, level1_load_factor))),
        ...
        spare = new hashTable_Aligned<uint64_t, 4>(ceil(5.0 * max_number_of_elements / (1.0 * ceil_log2(max_number_of_elements))),
    ...
    inline auto lookup(const itemType s) const -> bool {
        ...
        // bool spare_res = spare->find(spare_val);
        auto res = Wrap_Fixed_pd::find(quot, rem, header, body);
        return (res != -1) ? res : (spare->find(((uint64_t) pd_index << (quotient_length + bits_per_item)) | (quot << bits_per_item) | rem));
    }
 ...
 */
    ...
    void insert(const itemType s) {
        ...
        // assert(map_db.find(s) == map_db.end());
        // map_db[s] = s;
        uint64_t hash_res = Hasher(s);
        // std::cout << "s: \t" << s;
        // std::cout << "|\t hash_res: " << hash_res << std::endl;
        // uint64_t hash_res = s;
        uint32_t out1 = hash_res >> 32u, out2 = hash_res & MASK32;
        const bool ind = out2 & (1 << 16);
        const uint32_t pd_index = reduce32(out1, (uint32_t) number_of_pd);
        ...
        if (res == -1) {
            ...
            uint64_t partial_spare_item = ((uint64_t) pd_index << (quotient_length + bits_per_item)) | (quot << bits_per_item);
            promote_quot_and_remove_from_body(quot, fixed_pd::max_valide_counter_value, partial_spare_item, header, body);
            ...
            assert(temp_val);
            uint64_t partial_spare_item = ((uint64_t) pd_index << (quotient_length + bits_per_item)) | (max_quot << bits_per_item);
    ...
    void promote_quot_and_remove_from_body(uint64_t quot, size_t curr_counter_val, uint64_t partial_spare_item, uint64_t *header, __m512i *body) {
    ...
    void promote_quot_and_remove_from_body(uint64_t quot, uint64_t partial_spare_item, uint64_t *header, __m512i *body) {
    ...
    auto get_capacity() -> size_t {
        ...
        for (int i = 0; i < number_of_pd; ++i) {
            res += fixed_pd::Header::get_capacity(packed_fpd_array[i].header1) + fixed_pd::Header::get_capacity(packed_fpd_array[i].header2);
