...
#ifndef FILTERS_DICT256_VER6_DB_HPP
#define FILTERS_DICT256_VER6_DB_HPP
...
class Dict256_Ver6_DB {
    enum lookup_correct_val {
        Yes = true, No = false, IDK = -42
    };
...
public:
    Dict256_Ver6_DB(size_t max_number_of_elements, double level1_load_factor, double level2_load_factor)
...
//        return lookup2(s);
        ...
        if (should_look_in_l1)
            return lookup_l1_safe(s, correct_val);
        return lookup_l2_safe(s, correct_val);
    }
...
//        assert(lookup_exact(s, true));
    ...
    item_key_t spare_pop(uint64_t pd_index) {
        item_key_t item = test_for_equality_of_the_pop_item(pd_index);
        auto vv_pop_item = Level2_Log->pop(pd_index);
        auto spare_vv_pop_item = spare->pop(pd_index);
        if (spare_vv_pop_item.pd_index != -1)
            flip_quot(&spare_vv_pop_item);
        assert(vv_pop_item == item);
        assert(spare_vv_pop_item == item);
        return item;
    }
...
#endif//FILTERS_DICT256_VER6_DB_HPP
