...
void auto_rates(size_t n, double eps, size_t insertion_reps, size_t lookup_reps, bool is_adaptive, bool call_adapt,
    ...
    if (to_cout) {
        if (remote_opt)
            rates_attempt(n, eps, insertion_reps, lookup_reps, is_adaptive, call_adapt, cout);
        else if (naive_check)
            naive_rates(n, eps, insertion_reps, lookup_reps, is_adaptive, call_adapt, cout);
        else
            rates(n, eps, insertion_reps, lookup_reps, is_adaptive, call_adapt, cout);
        return;
    }
...
//    cout << "temp path is: " << temp_path << endl;
    ...
    if (remote_opt)
        rates_attempt(n, eps, insertion_reps, lookup_reps, is_adaptive, call_adapt, os);
    else if (naive_check)
        naive_rates(n, eps, insertion_reps, lookup_reps, is_adaptive, call_adapt, os);
    else
        rates(n, eps, insertion_reps, lookup_reps, is_adaptive, call_adapt, os);
    file.close();
...
rates_attempt(size_t n, double eps, size_t insertion_reps, size_t lookup_reps, bool is_adaptive, bool call_adapt,
    ...
    t0 = clock();
    for (auto iter : member_set) w.add(&iter, 0, true);
    double insertion_time = (clock() - t0) / ((double) CLOCKS_PER_SEC);
