...
class att_d512
{
    ...
    vector<uint8_t> pd_capacity_vec;
...
public:
    att_d512(size_t max_number_of_elements, double level1_load_factor, double level2_load_factor)
        : filter_max_capacity(max_number_of_elements),
          ...
          pd_index_length(ceil_log2(number_of_pd)),
 ...
 */
    ...
    void old_insert(const itemType s)
    {
        ...
        if (ans > 0)
        {
            ...
            std::cout << "pd_capacity_vec[pd_index - 1]: " << pd_capacity_vec[pd_index - 1] << "(" << pd_index - 1 << ")" << std::endl;
            std::cout << "pd_capacity_vec[pd_index]: " << pd_capacity_vec[pd_index] << "(" << pd_index << ")" << std::endl;
            std::cout << "pd_capacity_vec[pd_index + 1]: " << pd_capacity_vec[pd_index + 1] << "(" << pd_index + 1 << ")" << std::endl;
            assert(false);
        }
        if (ans == 0)
